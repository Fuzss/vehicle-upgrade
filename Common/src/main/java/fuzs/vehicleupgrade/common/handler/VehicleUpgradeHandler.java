...
import fuzs.vehicleupgrade.common.config.LeavesMountCollisions;
...
public class VehicleUpgradeHandler {
    ...
    private static @Nullable Entity getTraversableEntity(@Nullable Entity entity) {
        if (entity != null) {
            if (entity.hasControllingPassenger() && entity.is(ModRegistry.TRAVERSING_MOUNTS_ENTITY_TYPE_TAG)) {
                return entity;
            } else if (entity.isPassenger() && entity.getVehicle() != null && entity.getVehicle()
                    .is(ModRegistry.TRAVERSING_MOUNTS_ENTITY_TYPE_TAG)) {
